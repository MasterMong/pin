...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $jsonAttachmentType = json_decode(File::get(base_path('data/attchmentTypes.json')), true);
        foreach ($jsonAttachmentType as $key => $value) {
            self::createIfEmpty($value, 'area_attachment_types', 'name');
        }
    ...
    public static function createIfEmpty($data, $table, $key = 'id')
    {
        ...
        if (empty($exits)) {
            ...
            foreach ($keys as $i => $key) {
                $data[$key] = is_array($data[$key]) ? json_encode($data[$key]) : $data[$key];
            }
            try {
